...
export class CharacterSelectionScreen {
    ...
    showCharacterTab(tabType) {
        ...
        // Update tab buttons
        const defaultBtn = document.getElementById('defaultCharactersTab');
        const customBtn = document.getElementById('customCharactersTab');
    ...
    displayCharacters(characters, isCustom) {
        ...
        if (Object.keys(characters).length === 0) {
            ...
            characterGrid.appendChild(message);
            return;
        }
    ...
    selectCharacterCard(cardElement, characterId, character) {
        ...
        // Show info bar
        const infoBar = document.getElementById('characterInfoBar');
    ...
    selectCharacter() {
            ...
            // Add mod indicator if this is a mod character
            const modIndicator = character.isModCharacter ? 
                `<div style="background: #00aa00; color: white; font-size: 10px; padding: 2px 6px; border-radius: 10px; display: inline-block; margin-bottom: 5px;">🔧 MOD: ${character.modName}</div>` : '';
